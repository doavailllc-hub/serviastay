    ...
    <div className="min-h-screen bg-[#FAFAFC]">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 py-10">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <PreferenceBox
            title="Languages"
            icon={<Globe className="text-[#8363F5]" />}
            items={languages}
          ...
          <PreferenceBox
            title="Currency"
            icon={<DollarSign className="text-[#8363F5]" />}
            items={currencies}
        ...
        <div className="mt-10 flex justify-end">
          <button
            ...
            className="px-8 py-4 rounded-xl bg-[#8363F5] hover:bg-[#7152E8] text-white font-semibold shadow-lg transition"
          >
            Save Preferences
          </button>
        </div>
        ...
        {items.map((item) => (
          <button
            ...
            className={`w-full flex items-center justify-between rounded-xl border px-5 py-4 transition ${
              selected === item
                ? "border-[#8363F5] bg-[#F4F1FF]"
                : "border-gray-200 hover:border-[#8363F5]"
            }`}
          >
            <span className="font-medium">{item}</span>

            {selected === item && (
              <Check size={20} className="text-[#8363F5]" />
            )}
          </button>
        ))}
      </div>
