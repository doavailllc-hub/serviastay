...
function Avatar({ name }) {
  ...
  return (
    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#EEE9FF] text-sm font-black text-[#8363F5]">
      {letter}
    </div>
  );
}
...
function RoleSelect({ value, onChange, fullWidth = false }) {
  return (
    <select
      value={value}
      onChange={(e) => onChange(e.target.value)}
      className={`h-10 rounded-xl border border-gray-200 bg-gray-50 px-3 text-sm font-bold capitalize text-gray-700 outline-none transition focus:border-[#8363F5] focus:bg-white focus:ring-4 focus:ring-[#8363F5]/10 ${
        fullWidth ? "w-full" : ""
      }`}
    >
      <option value="guest">Guest</option>
